...
string net2check_fn(bool qry, const vector<string>& args, runtime& rte ) {
    ...
    // Query should be possible always, command only when
    // doing nothing or net2check
    INPROGRESS(rte, reply, !(qry || ctm==no_transfer || ctm==net2check))

    // Good. See what the usr wants
    if( qry ) {
        reply << " 0 : ";
        if( rte.transfermode!=net2check ) {
            reply << "inactive : 0";
        } else {
            reply << "active : 0";
        }
    ...
    if( args[1]=="open" ) {
        ...
        if( rte.transfermode!=no_transfer ) {
            ...
            rte.netparms.set_host( hosts[&rte] );
