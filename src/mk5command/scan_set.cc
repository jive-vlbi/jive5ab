...
string scan_set_fn(bool q, const vector<string>& args, runtime& rte) {
    ...
    INPROGRESS(rte, reply, !(q || diskunavail(ctm) || ctm==no_transfer))
    ...
    for ( unsigned int argument_position = 2; argument_position < min((size_t)4, args.size()); argument_position++ ) {
        ...
        if ( is_time ) {
            ...
            if ( !data_checked ) {
                ...
                if ( !find_data_format( (unsigned char*)buffer->data, bytes_to_read, track, true, rte.scan_check_config.verbose, found_data_type) ) {
                    failed = true;
                }
                    ...
                    if ( !is_data_format( (unsigned char*)buffer->data, bytes_to_read, track, header_format, true, rte.scan_check_config.verbose, end_data_type) ) {
                        failed = true;
                    }
