...
// The disk2etransfer 'guard' or 'finally' function
void disk2etransferguard_fun(runtime* rteptr, chain::stepid s) {
    try {
        DEBUG(3, "disk2etransfer guard function: transfer done" << endl);
        ...
        DEBUG(-1, "disk2etransfer finalization threw an exception: " << e.what() << std::endl );
    }
    catch ( ... ) {
        DEBUG(-1, "disk2etransfer finalization threw an unknown exception" << std::endl );        
    }
...
string disk2etransfer_fn( bool qry, const vector<string>& args, runtime& rte) {
    ...
    // <connect>
    //
    //  disk2etransfer = connect : <host> : 
    //     <host> is address of a machine running e-transfer daemon 
    if( args[1]=="connect" ) {
        recognized = true;
        ...
        if( rte.transfermode==no_transfer ) {
            ...
            if( fromfile(rtm) || fromdisk(rtm) )
                c.register_final(&disk2etransferguard_fun, &rte, netstep);
    ...
    if( args[1]=="on" ) {
        ...
        if( ((rte.transfermode==disk2net  || rte.transfermode==file2net) && rte.transfersubmode&connected_flag)
            ...
            if( args.size()>2 && !args[2].empty() ) {
                ...
                rte.processingchain.register_final(&disk2etransferguard_fun, &rte, chain::invalid_stepid);
