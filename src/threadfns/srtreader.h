...
//          7990 AA Dwingeloo
//
#ifndef JIVE5A_THREADFNS_SRTREADER_H
#define JIVE5A_THREADFNS_SRTREADER_H
...
void srtreader(outq_type<Item>* outq, sync_type<fdreaderargs>* args) {
    bool                   stop;
    uint64_t               bytesread;
    runtime*               rteptr;
    ...
    RTEEXEC(*rteptr,
            ...
            rteptr->statistics.init(args->stepid, "SrtReadv1"));
    ...
    if( stop ) {
        DEBUG(0, "srtreader: stop signalled before we actually started" << std::endl);
        return;
    }

    DEBUG(0, "srtreader: read fd=" << network->fd << " rd:" << rd_size
    ...
    while( !stop ) {
        ...
        while( !stop && (ptrdiff_t)(eptr-ptr)>=(ptrdiff_t)wr_size ) {
            ...
            while( nrec!=rd_size ) {
                if( (r=srt::UDT::recv(network->fd, ((char*)ptr)+nrec, rd_size-nrec, 0))==srt::CUDT::ERROR ) {
                    srt::UDT::ERRORINFO& srterror = srt::UDT::getlasterror();
                    DEBUG(0, "srtreader: error " << srterror.getErrorMessage() << " (" 
                              << srterror.getErrorCode() << ")" << std::endl);
        ...
        if( ptr!=eptr ) {
            if( !network->allow_variable_block_size ) {
                DEBUG(-1, "srtreader: skip blok because of constraint error. blocksize not integral multiple of write_size" << std::endl);
                continue;
            }
            ...
            DEBUG(3, "srtreader: partial block; adjusting block size by -" << (size_t)(eptr - ptr) << std::endl);
        }
        // push it downstream
    ...
    DEBUG(0, "srtreader: stopping. read " << bytesread << " (" <<
