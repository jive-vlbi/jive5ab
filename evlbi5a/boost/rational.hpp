...
#include <boost/type_traits/is_unsigned.hpp> // for boost::is_unsigned ...
...
#pragma parse_mfunc_templ reset
    ...
    template <typename OtherRationalType>
    OtherRationalType as( void ) const {
        return OtherRationalType( typename OtherRationalType::int_type(num),
                                  typename OtherRationalType::int_type(den) );
    }
...
// g++ complains loudly about
//   while ( r<0 ) { ... }
// for rational<unsigned> if compiled with
// "-W -Wall -Werror -Wextra" about
// "comparison of unsigned expression < 0 is always false"
// so temporarily disable this warning
// Unfortunately GCC on debian etch does not support this
// construction:
//      #ifdef __GNUC__
//      #pragma GCC diagnostic push
//      #pragma GCC diagnostic ignored "-Wtype-limits"
//      #endif
// so we'll have to come up with something different and portable *sigh*

// The boolean parameter should indicate wether we're dealing with
// an unsigned type or not. We can implement the generic case (
// with the "while ( ... < 0 ) { ... }" and then specialize the
// 'true' case to do nothing [for unsigned types there is nothing
// to fix up]
template <bool, typename T>
struct fixup_negative {
    static void fix(T& r, T& q, T const& d, T const& zero) {
        while ( r<zero ) { r += d; --q; }
    }
};

template <typename T>
struct fixup_negative<true, T> {
    static void fix(T&, T&, T const&, T const& ) { }
};
...
{
    typedef fixup_negative< ::boost::is_unsigned<IntType>::value, int_type> fixup_type;
    // Avoid repeated construction
    ...
    fixup_type::fix(ts.r, ts.q, ts.d, zero);
    fixup_type::fix(rs.r, rs.q, rs.d, zero);
...
bool rational<IntType>::operator< (param_type i) const
{
    ...
    fixup_type::fix(r, q, this->den, zero);
...
// See above; this doesn't work on Debian Etch gcc ...
//#ifdef __GNUC__
...
std::istream& operator>> (std::istream& is, rational<IntType>& r)
{
    ...
    if (is)
        ...
        throw std::runtime_error( "Badly formed rational" );
