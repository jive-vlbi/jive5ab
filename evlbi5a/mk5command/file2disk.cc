...
typedef per_runtime<f2d_data_type>  f2d_data_store_type;

// The guard function which finalizes the "file2disk" transfer
void file2diskguard_fun(runtime* rteptr, f2d_data_store_type::iterator p) {
    try {
        ...
        rteptr->xlrdev.finishScan( p->second.scan_pointer );
...
string file2disk_fn(bool qry, const vector<string>& args, runtime& rte ) {
    static f2d_data_store_type  f2d_data;
    ostringstream               reply;
    ...
    if ( qry ) {
        ...
        f2d_data_store_type::const_iterator   ptr = f2d_data.find(&rte);
    ...
    f2d_data_store_type::iterator ptr = f2d_data.insert( make_pair(&rte, f2d_data_type()) ).first; 
    f2d_data_type&                f2d( ptr->second );
    ...
    c.register_final(&file2diskguard_fun, &rte, f2d_data.find(&rte));
