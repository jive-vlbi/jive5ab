...
int srt_startup() { return srt::CUDT::startup(); }
int srt_cleanup() { return srt::CUDT::cleanup(); }

// Socket creation.
SRTSOCKET srt_socket(int , int , int ) { return srt::CUDT::socket(); }
SRTSOCKET srt_create_socket() { return srt::CUDT::socket(); }

#if ENABLE_BONDING
// Group management.
SRTSOCKET srt_create_group(SRT_GROUP_TYPE gt) { return srt::CUDT::createGroup(gt); }
SRTSOCKET srt_groupof(SRTSOCKET socket) { return srt::CUDT::getGroupOfSocket(socket); }
int srt_group_data(SRTSOCKET socketgroup, SRT_SOCKGROUPDATA* output, size_t* inoutlen)
{
    return srt::CUDT::getGroupData(socketgroup, output, inoutlen);
}
...
{
    return srt::CUDT::connectLinks(group, name, arraysize);
}
        ...
        const struct sockaddr* remote_name, int remote_namelen)
{
    ...
    if ( (local_name->sa_family != AF_INET && local_name->sa_family != AF_INET6)
        ...
        return srt::CUDT::APIError(MJ_NOTSUP, MN_INVAL, 0);
...
int srt_recv(SRTSOCKET u, char * buf, int len) { return srt::CUDT::recv(u, buf, len, 0); }
int srt_sendmsg(SRTSOCKET u, const char * buf, int len, int ttl, int inorder) { return srt::CUDT::sendmsg(u, buf, len, ttl, 0!=  inorder); }
int srt_recvmsg(SRTSOCKET u, char * buf, int len) { int64_t ign_srctime; return srt::CUDT::recvmsg(u, buf, len, ign_srctime); }
int64_t srt_sendfile(SRTSOCKET u, const char* path, int64_t* offset, int64_t size, int block)
{
    ...
    if (!ifs)
    {
        return srt::CUDT::APIError(MJ_FILESYSTEM, MN_READFAIL, 0);
    }
...
int64_t srt_recvfile(SRTSOCKET u, const char* path, int64_t* offset, int64_t size, int block)
{
    ...
    if (!ofs)
    {
        return srt::CUDT::APIError(MJ_FILESYSTEM, MN_WRAVAIL, 0);
    }
...
int srt_sendmsg2(SRTSOCKET u, const char * buf, int len, SRT_MSGCTRL *mctrl)
{
    ...
    if (mctrl)
        return srt::CUDT::sendmsg2(u, buf, len, (*mctrl));
    SRT_MSGCTRL mignore = srt_msgctrl_default;
    return srt::CUDT::sendmsg2(u, buf, len, (mignore));
}

int srt_recvmsg2(SRTSOCKET u, char * buf, int len, SRT_MSGCTRL *mctrl)
{
    if (mctrl)
        return srt::CUDT::recvmsg2(u, buf, len, (*mctrl));
    SRT_MSGCTRL mignore = srt_msgctrl_default;
    return srt::CUDT::recvmsg2(u, buf, len, (mignore));
}
...
int srt_bstats(SRTSOCKET u, SRT_TRACEBSTATS * perf, int clear) { return srt::CUDT::bstats(u, perf, 0!=  clear); }
int srt_bistats(SRTSOCKET u, SRT_TRACEBSTATS * perf, int clear, int instantaneous) { return srt::CUDT::bstats(u, perf, 0!=  clear, 0!= instantaneous); }

SRT_SOCKSTATUS srt_getsockstate(SRTSOCKET u) { return SRT_SOCKSTATUS((int)srt::CUDT::getsockstate(u)); }

// event mechanism
int srt_epoll_create() { return srt::CUDT::epoll_create(); }

int srt_epoll_clear_usocks(int eit) { return srt::CUDT::epoll_clear_usocks(eit); }
...
int srt_epoll_add_usock(int eid, SRTSOCKET u, const int * events) { return srt::CUDT::epoll_add_usock(eid, u, events); }
...
int srt_epoll_add_ssock(int eid, SYSSOCKET s, const int * events)
{
    ...
    return srt::CUDT::epoll_add_ssock(eid, s, &flag);
}

int srt_epoll_remove_usock(int eid, SRTSOCKET u) { return srt::CUDT::epoll_remove_usock(eid, u); }
int srt_epoll_remove_ssock(int eid, SYSSOCKET s) { return srt::CUDT::epoll_remove_ssock(eid, s); }
...
int srt_epoll_update_ssock(int eid, SYSSOCKET s, const int * events)
{
    ...
    // call UDT native function
    return srt::CUDT::epoll_update_ssock(eid, s, &flag);
}
...
int32_t srt_epoll_set(int eid, int32_t flags) { return srt::CUDT::epoll_set(eid, flags); }

int srt_epoll_release(int eid) { return srt::CUDT::epoll_release(eid); }
...
{
    return srt::CUDT::socketStartTime(sock);
}
